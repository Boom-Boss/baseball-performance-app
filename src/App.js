...
import { getFirestore, doc, setDoc, getDoc, collection, addDoc, query, where, onSnapshot, serverTimestamp, orderBy, writeBatch, deleteDoc, updateDoc } from 'firebase/firestore';
...
function CoachView({ coachId, appId, setView, allPlayers }) {
    ...
    const coachTabs = ['dashboard', 'reports', 'team', 'drill library', 'goals', 'notes', 'throwing', 'lifting', 'nutrition', 'messaging'];
                     ...
                     activeTab === 'drill library' ? <DrillLibrary coachId={coachId} appId={appId} /> : (
                        selectedPlayer ? (
                            <div>
                                ...
                                {activeTab === 'goals' && <Goals player={selectedPlayer} coachId={coachId} appId={appId} isPlayerView={false} />}
                                {activeTab === 'notes' && <Notes player={selectedPlayer} coachId={coachId} appId={appId} />}
...
function PlayerView({ player, coachId, appId, setView }) {
    const [activeTab, setActiveTab] = useState('dashboard');
    const playerTabs = ['dashboard', 'wellness log', 'team', 'drill library', 'goals', 'throwing', 'lifting', 'messaging'];
    return (
        <div className="bg-gray-900 text-white min-h-screen font-sans">
            <div className="flex flex-col md:flex-row">
                <aside className="w-full md:w-64 bg-gray-800 p-4 md:min-h-screen">
                ...
                <main className="flex-1 p-4 md:p-8">
                    <div>
                        ...
                        {activeTab === 'goals' && <Goals player={player} coachId={coachId} appId={appId} isPlayerView={true} />}
...
function Reports({ player, coachId, appId }) {
    ...
    const nutritionConsistencyData = [
        { name: 'Hit Goals', value: wellnessLogs.filter(l => l.hitCalories === 'Yes' && l.hitProtein === 'Yes').length },
        { name: 'Missed Goals', value: wellnessLogs.filter(l => l.hitCalories === 'No' || l.hitProtein === 'No').length }
    ];
        ...
        <div className="space-y-8">
            <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="bg-gray-800 p-6 rounded-lg shadow-lg md:col-span-2">
                    <h3 className="text-xl font-semibold mb-4 text-blue-300">Nutrition Goal Consistency</h3>
                    <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
                            <Pie data={nutritionConsistencyData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} fill="#8884d8" label>
                                {nutritionConsistencyData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                            </Pie>
                            <Tooltip contentStyle={{ backgroundColor: '#2d3748', border: 'none' }} />
                            <Legend />
                        </PieChart>
...
function Goals({ player, coachId, appId, isPlayerView }) {
    const [goals, setGoals] = useState([]);
    const [newGoal, setNewGoal] = useState({ title: '', metric: 'lbs', target: 100, current: 0 });

    const goalsDocRef = doc(db, `artifacts/${appId}/users/${coachId}/players/${player.id}/programs/goals`);

    useEffect(() => {
        const unsubscribe = onSnapshot(goalsDocRef, (doc) => {
    ...
    const handleAddGoal = async () => {
        ...
        try {
            ...
            setNewGoal({ title: '', metric: 'lbs', target: 100, current: 0 });
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
                <h3 className="text-xl font-semibold mb-4 text-blue-300">Player Goals</h3>
                <div className="space-y-4">
                    {goals.map((goal, index) => (
                        <div key={index} className="bg-gray-700 p-4 rounded-lg">
                            <div className="flex justify-between items-center">
                                <span className="font-semibold">{goal.title}</span>
                                {!isPlayerView && <button onClick={() => handleDeleteGoal(index)} className="text-red-500 text-xs">Delete</button>}
                            </div>
                            <div className="flex items-center gap-4 mt-2">
                                <input type="number" value={goal.current} onChange={e => handleGoalChange(index, 'current', Number(e.target.value))} className="bg-gray-600 p-1 rounded w-24 text-center"/>
                                <div className="w-full bg-gray-600 rounded-full h-4">
                                    <div className="bg-green-500 h-4 rounded-full" style={{ width: `${(goal.current / goal.target) * 100}%` }}></div>
                                </div>
                ...
                {!isPlayerView && (
                    <div className="mt-6 p-4 border-t border-gray-700">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                            <input type="text" value={newGoal.title} onChange={e => setNewGoal({...newGoal, title: e.target.value})} placeholder="Goal Title (e.g., Max Squat)" className="bg-gray-700 p-2 rounded col-span-2"/>
                            <input type="number" value={newGoal.target} onChange={e => setNewGoal({...newGoal, target: Number(e.target.value)})} placeholder="Target" className="bg-gray-700 p-2 rounded"/>
                            <input type="text" value={newGoal.metric} onChange={e => setNewGoal({...newGoal, metric: e.target.value})} placeholder="Metric (lbs, mph)" className="bg-gray-700 p-2 rounded"/>
                        </div>
                        <button onClick={handleAddGoal} className="w-full mt-2 bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">Add Goal</button>
                    </div>
                )}
                 <button onClick={saveGoals} className="w-full mt-4 bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Save Progress</button>
            </div>
        </div>
    );
}

function Notes({ player, coachId, appId }) {
    const [notes, setNotes] = useState([]);
    const [newNote, setNewNote] = useState('');
    const notesDocRef = doc(db, `artifacts/${appId}/users/${coachId}/players/${player.id}/programs/notes`);

    useEffect(() => {
        const unsubscribe = onSnapshot(notesDocRef, (doc) => {
    ...
    const handleAddNote = async () => {
        ...
        try {
            await setDoc(notesDocRef, { notes: updatedNotes });
            setNewNote('');
        } catch (error) {
    ...
    return (
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
            <h3 className="text-xl font-semibold mb-4 text-blue-300">Private Coach's Notes</h3>
            <div className="space-y-4">
                <textarea value={newNote} onChange={e => setNewNote(e.target.value)} rows="4" placeholder="Add a new timestamped note..." className="bg-gray-700 p-2 rounded w-full"></textarea>
                <button onClick={handleAddNote} className="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">Add Note</button>
            </div>
